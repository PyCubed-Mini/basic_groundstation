...
class RFM9x:
    ...
    async def send(
        ...
        *,
        ...
        self._write_u8(Constants._RH_RF95_REG_0D_FIFO_ADDR_PTR, 0x00)  # FIFO starts at 0.
    ...
    async def send_with_ack(self, data: ReadableBuffer) -> bool:
        ...
        while not got_ack and retries_remaining:
            ...
            self.flags |= Constants._RH_FLAGS_RETRY
        ...
        if not timed_out:
            if self.enable_crc and self.crc_error():
                ...
                if fifo_length < 5:
                    ...
                    if (
                        self.node != Constants._RH_BROADCAST_ADDRESS
                        and packet[1] != Constants._RH_BROADCAST_ADDRESS
                    ...
                    # send ACK unless this was an ACK or a broadcast
                        ...
                        with_ack
                        and ((packet[4] & Constants._RH_FLAGS_ACK) == 0)
                        and (packet[1] != Constants._RH_BROADCAST_ADDRESS)
                    ):
                        ...
                        await self.send(
                            ...
                            node=packet[0],
                            identifier=packet[2],
                            flags=(packet[4] | Constants._RH_FLAGS_ACK),
                        )
                        # reject Retries if we have seen this idetifier from this source before
                        if (self.seen_ids[packet[1]] == packet[2]) and (
                            packet[4] & Constants._RH_FLAGS_RETRY
                        ):
                            packet = None
